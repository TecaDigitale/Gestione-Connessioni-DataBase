...
public class Navigator {
	 ...
	 * 
	 */
	protected String viewNavigatore(Hashtable<String, Column> campi) {
		String ris = "";
	 ...
	 * 
	 */
	protected MessageElement viewNavigatore(Hashtable<String, Column> campi,
			Hashtable<String, Column> campiOri, ConvertToUTF8 convert,
	 ...
	 * 
	 */
	public String getQueryString(Hashtable<String, Column> campi) {
		String ris = "";
		String key = "";
		Column column = null;
		...
		for (Enumeration<String> e = campi.keys(); e.hasMoreElements();) {
			key = e.nextElement();
			column = campi.get(key);
			if (!column.isEmpty()) {
				if (!ris.endsWith("&"))
					ris += ris.equals("") ? "" : "&";
				if (column.getValue().getClass().getName()
						.equals("java.lang.Integer"))
					ris += addQueryString(key,
							((Integer) column.getValue()).intValue());
				else if (column.getValue().getClass().getName()
						.equals("java.lang.String"))
					ris += addQueryString(key, (String) column.getValue());
				else if (column.getValue().getClass().getName()
						.equals("java.util.GregorianCalendar")
						|| column.getValue().getClass().getName()
					...
					if (column.getValue().getClass().getName()
						...
						gc.setTimeInMillis(((Timestamp) column.getValue())
								.getTime());
					}
	 ...
	 * 
	 */
	private MessageElement getXml(Hashtable<String, Column> campi, Hashtable<String, Column> campiOri,
		...
		try {
			...
			for (Enumeration<String> e = campiOri.keys(); e.hasMoreElements();) {
				key = e.nextElement();
				column = campi.get(key.toLowerCase());
				if (!column.isEmpty()) {
					if (column.getValue().getClass().getName()
							.equals(Integer.class.getName()))
						convert.addChildElement(campiXml, nodes, "campo",
								Integer.toString(((Integer) column.getValue())
										.intValue()), "key", key, true, true);
					else if (column.getValue().getClass().getName()
							.equals(String.class.getName()))
						convert.addChildElement(campiXml, nodes, "campo",
								(String) column.getValue(), "key", key, true,
								true);
					else if (column.getValue().getClass().getName()
	 ...
	 * 
	 */
	private String getHidden(Hashtable<String, Column> campi) {
		String ris = "";
		String key = "";
		Column column = null;
		GregorianCalendar gc = null;
