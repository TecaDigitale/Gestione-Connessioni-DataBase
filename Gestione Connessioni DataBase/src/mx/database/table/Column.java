...
public class Column
{
	...
	private static Logger log = new Logger(Column.class, "mx.database");
	/**
   ...
   */
	...
	/**
   * Questa variabile viene utilizzata per indicare il nome della colonna nella
   * tabella
   */
   ...
   */
	...
	/**
   * Questa variabile viene utilizzata per indicare il codice da utilizzare per
   * il calcolo della chiave primaria
   */
	private String codeID = "";

	/**
   * Questa variabile viene utilizzata per indicare la lunghezza della chiave da
   * utilizzare per il calcolo della chiave primaria
   */
   ...
   */
	...
	/**
   * Questa variabile viene utilizzata per forzare il campo nella condizione di
   * Where
   */
   ...
   */
	...
	/**
   * Questa variabile viene utilizzata per indicare il tipo di operatore boolean
   ...
   */
	...
	/**
   * variabile statica utilizzata per indicare che il campo non viene utilizzato
   * per l'ordinamento della select
   */
	public static int ORDERBY_NONE = 0;

	/**
   * variabile statica utilizzata per indicare che il campo viene utilizzato per
   * l'ordinamento crescente della select
   */
   ...
   *          (default: true)
   */
	public Column(String table, String column, boolean primaryKey, boolean where,
			boolean view)
	{
		super();
		this.table = table;
   ...
   */
	...
	/**
   * Questo metodo viene utilizzata per leggere il tipo di operatore boolean da
   * adottare nella where.<BR>
   * Default AND
   * 
   ...
   */
	...
	/**
   * Questo metodo viene utilizzata per indicare il tipo di operatore boolean da
   * adottare nella where.<BR>
   * Default AND
   * 
   * @param operatore
   ...
   * Questo metodo viene utilizzata per leggere il contenuto della colonna
   * 
   * @param encodingUtf8 Questa variabile viene utilizzata per definire se utilizzare la codifica in utf-8
   * @return Returns the value.
   */
	...
	public String getValueSql(boolean encodingUtf8)
	{
		...
		if (value.getClass().getName().equals("java.lang.String"))
		{
			...
			try
			{
				log.debug("Convert - 1: "+(new String(myValue.getBytes(),"UTF8")));
				log.debug("Convert - 2: "+(new String(myValue.getBytes("UTF8"))));
				log.debug("Convert - 3: "+(new String(myValue.getBytes("UTF8"),"UTF8")));
				log.debug("Convert - 4: "+(new String(myValue.getBytes(),"UTF-8")));
				log.debug("Convert - 5: "+(new String(myValue.getBytes("UTF-8"))));
				log.debug("Convert - 6: "+(new String(myValue.getBytes("UTF-8"),"UTF-8")));
			}
		...
		else if (value.getClass().getName().equals("java.util.GregorianCalendar"))
			ris = "'"
					+ MsSql.conveDateTimeEng(new Timestamp(((GregorianCalendar) value)
							.getTimeInMillis())) + "'";
   ...
   * 
   * @param orderBy
   *          Questo campo viene utilizzato per indicare il tipo di Ordinamento
   *          (Column.ORDERBY_DESC, Column.ORDERBY_NONE, Column.ORDERBY_CRES)
   * @param orderByPos
   ...
   */
	...
	/**
   * Questo metodo viene utilizzato per leggerela condizione da adottare per la
   * ricerca.<BR>
   * Default "="
   * 
   ...
   */
	...
	/**
   * Questo metodo viene utilizzata per valorizzare il campo nella condizione di
   * Where
   * 
   * @param forceWhere
   */
