...
public abstract class Table extends DataSet
{
	 ...
	 */
	private static Logger log = new Logger(Table.class,"mx.database");
  ...
  @SuppressWarnings("unchecked")
	public Table(ConnectionPool conn, MsSqlPool msp, Map map)
  {
    super(conn, msp, map);
  }
   ...
   */
  @SuppressWarnings("unchecked")
	public Table(ConnectionPool conn, Map map)
  {
    super(conn, map);
  }
  ...
  /**
   * Questo metodo viene utilizzato per implementare i controlli da eseguire prima dell'esecuzuibne delle operazioni di insert e update
   *
   */
  protected abstract void preUpdate();
  
  /**
   * Questo metodo viene utilizzato per implementare i controlli da eseguire dopo dell'esecuzuibne delle operazioni di insert e update
   *
   */
   ...
   * Nel caso in cui la colonna di tipo chiave primaria avesse il valore nullo o
   * 0 viene eseguito l'inserimeto di un nuovo record dopo aver generato una
...
//        msp = this.msp;
      ...
      if (cols !=null)
      {
        for(int x=0; x<cols.size(); x++)
        {
          ...
          if (col.getValue()==null)
            col.genNewID();
          else if (col.getValue().getClass().getName().equals("java.lang.Integer"))
          {
        ...
        throw new MsSqlException("Non risulta indicato alcuna chiave primaria");
      for (Enumeration iter = campi.keys(); iter.hasMoreElements();)
      {
        ...
        if (col.getValue()!=null && col.isView())
        {
          ...
          if (sql.equals(""))
          {
            sql = "INSERT INTO "+col.getTable()+" ("+col.getColumn();
...
//          conn.releaseConn(msp);
   ...
   * nuova chiave primaria, altrimenti si procede con l'aggiornamento.
   * 
   * @param writeNull Questa variabile viene utilizzata per forzare l'aggiornamento del campi null
   * @throws MsSqlException
   */
  @SuppressWarnings("unchecked")
...
//        msp = this.msp;
      for (Enumeration iter = campi.keys(); iter.hasMoreElements();)
      {
        key = (String) iter.nextElement();
        col = (Column) campi.get(key);
        if ((col.getValue()!=null || writeNull) && (col.getColumn() != null && !col.getColumn().equals("")))
        {
      		log.debug("update encodingUtf8: "+encodingUtf8);
          if (col.isPrimaryKey() || col.isForceWhere())
          {
            sql2 += sql2.equals("")?" WHERE ":" AND ";
            sql2 += col.getColumn()+"="+col.getValueSql(encodingUtf8);
          }
          else
          {
            sql += sql.equals("")?"UPDATE "+col.getTable()+" SET ":", ";
            sql += col.getColumn()+"="+(col.getValue()==null?"NULL":col.getValueSql(encodingUtf8));
          }
        }
      }
      sql = sql+sql2;
...
//          conn.releaseConn(msp);
  ...
  /**
   * Questo metodo viene utilizzato per canellare uno o più record dalla tabella
   * 
      ...
      if (cols !=null)
      {
        for(int x=0; x<cols.size(); x++)
        {
          ...
          if (col.isEmpty() && (!force))
            throw new MsSqlException("Campi chiave primaria non valorizzati");
          else
          {
            sql += sql.equals("")?"DELETE FROM "+col.getTable():"";
            if (!col.isEmpty() || col.getTipoRicerca().trim().equalsIgnoreCase("is null"))
            {
            	sql += isWhere?" AND ":" WHERE ";
            	if (col.getTipoRicerca().trim().equalsIgnoreCase("is null"))
            		sql += col.getColumn()+col.getTipoRicerca();
            	else
            		sql += col.getColumn()+col.getTipoRicerca()+col.getValueSql();
            	isWhere= true;
            }
          }
        }
...
//    }
  ...
  /**
   * Questo metodo viene utilizzato per eseguire l'operazione di insert, update, delete
   * @param sql Stringa SQL sa eseguire
